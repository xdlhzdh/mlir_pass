...
CASES = [
    ("matmul_add", ROOT / "test/matmul_add.mlir", ref_matmul_add),
    ("jit_scale", ROOT / "test/jit_scale.mlir", lambda: (np.array(
        [[1.0, 2.0], [3.0, 4.0]], dtype=np.float32) * 2.0).reshape(-1)),
]


def main() -> int:
    failed = 0
    for name, mlir_path, ref_fn in CASES:
